      ...
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tr v-for="(item, index) in loadedProjects" :key="index">
            <td>{{ item.name }}</td>
            <td>{{ item.enDescription.substring(0, 50) }}{{
                item.enDescription.length > 50 ? '...' : ''
              }}
            </td>
            <td>{{ item.ptDescription.substring(0, 50) }}{{
                item.ptDescription.length > 50 ? '...' : ''
              }}
            </td>
...
const handleSubmit = async () => {
  ...
  try {
    if (editingSkill.value) {
      await axios.put(`${API_BASE_URL}/v1/skills/${editingSkill.value.id}`, skill.value);
...
const handleProjectSubmit = async () => {
  ...
  try {
    ...
    if (editingProject.value) {
      ...
      await axios.post(`${API_BASE_URL}/v1/projects`, projectData);
      message.value = "Project added successfully!";
    }

    project.value = {name: "", enDescription: "", ptDescription: "", githubLink: "", tags: ""};
    await fetchProjects();
  } catch (error) {
...
const fetchSkills = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/v1/skills`);
...
const deleteSkill = async (skillId) => {
  if (confirm('Are you sure you want to delete this skill?')) {
    try {
      await axios.delete(`${API_BASE_URL}/v1/skills/${skillId}`);
